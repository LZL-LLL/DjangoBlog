...
def load_sidebar(user, linktype):
    ...
    if value:
        ...
        # 标签云 — 按文章数排序，显示全部标签，size = (count/avg)*5+10
        increment = 5
        ...
        if user and user.is_authenticated:
            ...
            user_skills = [s.strip() for s in user.skills.split(',') if s.strip()] if user.skills else []
        else:
            user_avatar = blogsetting.avatar.url if blogsetting.avatar else None
            user_bio = blogsetting.bio or ''
            user_location = blogsetting.location or ''
            user_skills = [s.strip() for s in blogsetting.skills.split(',') if s.strip()] if blogsetting.skills else []

        value['AVATAR_URL'] = user_avatar
        value['BIO'] = user_bio
        value['LOCATION'] = user_location
        value['PERSONAL_WEBSITE'] = blogsetting.personal_website or ''
        value['SKILLS'] = user_skills
